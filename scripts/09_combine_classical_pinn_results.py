...
The combined report is defensive: when the classical pipeline has not yet been
run, PINN-only outputs are still produced with a clear warning.
"""
...
def main() -> None:
    ...
    classical_raw = loaded["classical_raw"]
    pinn_raw = loaded["pinn_raw"]

    if classical_raw.empty:
        print(f"[combine] WARNING: classical raw_results.csv missing at {classical_dir}; "
              "combined report will contain PINN rows only.")
    if pinn_raw.empty:
        print(f"[combine] WARNING: PINN raw_results.csv missing at {pinn_dir}; "
              "combined report will contain classical rows only.")
    ...
    if "model" in combined_raw.columns and "architecture" not in combined_raw.columns:
        combined_raw["architecture"] = combined_raw["model"]
    ...
    # Publication main table: cell-macro MAE by architecture x preprocessing x target
    ...
    # Predictions merged (PINN only; classical predictions live under folds/).
    pinn_predictions = _read_parquet(pinn_dir / "all_predictions.parquet")
    if not pinn_predictions.empty:
        atomic_write_parquet(pinn_predictions, combined_dir / "prediction_metrics_combined.parquet")
